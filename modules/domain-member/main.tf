resource "azurerm_public_ip" "static" {
  count = 2
  name                = "${var.prefix}-client-pip-${count.index}"
...
resource "azurerm_network_interface" "primary" {
  count = 2
  name                = "${var.prefix}-client-nic-${count.index}"
  location            = var.location
  resource_group_name = var.resource_group_name
  ip_configuration {
    name                          = "primary"
    ...
    public_ip_address_id          = element(azurerm_public_ip.static[*].id, count.index)   
  }
}
...
resource "azurerm_virtual_machine_extension" "wait-for-domain-to-provision" {
  ...
  virtual_machine_id   = element(azurerm_windows_virtual_machine.domain-member[*].id, count.index)
  settings             = <<SETTINGS
  {
...
resource "azurerm_virtual_machine_extension" "join-domain" {
  count = 2
  name                 = element(azurerm_windows_virtual_machine.domain-member[*].name, count.index)
  publisher            = "Microsoft.Compute"
